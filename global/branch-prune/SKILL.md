...
description: >-
  ...
  branch — refusing on a dirty tree, an open PR, a live worktree, or unmerged
...
3. **Delete the two disposable buckets.** Local: `git branch -d` — plain `-d`,
   so git's own refusal is a second floor under the classification. Reach for
   `-D` only against an explicit ack for a named branch. Remote:
   `git push origin --delete` for the same set, **confirmed separately** — it
   publishes, and it is the half no local reflog can undo.
4. **Report the ledger** — one line per branch: deleted, or kept with its
   bucket as the reason.
