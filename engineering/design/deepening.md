...
2. **Local-substitutable**: a real local stand-in exists (PGLite for
   ...
   run against it, and the seam stays internal, with no port at the external
   interface.
3. **Remote but owned**: your own services across a network (internal APIs,
   ...
   and you inject the transport as an adapter, HTTP/queue in production,
...
- Price every proposed seam by its adapters. One adapter is a hypothetical
  seam, plain indirection, so hold the port until the second adapter (usually
  the test one) is justified.
...
- New tests go at the deepened module's interface, which is the test surface,
  asserting observable outcomes, not internal state.
