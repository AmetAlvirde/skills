...
context in hand. The point over bare `grill` is that you look facts up in the
code instead of asking the human for them.
...
read it first. Reuse the increment's `id`, note that context came from the repo,
and `[[link]]` the source map. Offer to commit, then close per `_saving.md`. Its
