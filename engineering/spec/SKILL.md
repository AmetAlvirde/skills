...
Run it as **@vitruv**. The definer holds Opus 5.5 high across the confirm→approve
...
2. **Confirm the seams before writing.** Name the seams the build will cut
   ...
   discipline's vocabulary. If a seam is still fuzzy, compose `converge` on that
   one seam; the lens is that a weak joint is a boundary the build can't cut
   along. Otherwise state each seam and get a nod.
...
Save per `~/Dev/notes/_saving.md`, and read it first. `artifact: spec`, reuse
the increment's `id`, `[[link]]` the source map and grills. If a durable,
