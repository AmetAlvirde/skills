...
- **Seal at log-out, not at midnight.** A session that runs `18:00 → 03:12`
  ...
  a midnight-keyed one fails on exactly the nights worth recording. On those
  nights the ritual competes with the work, and the work wins.
...
2. **Reads what the trackers recorded**: what closed this session, and what was
   ...
   **An issue closed this session is proven progress.** It clears the evidence
   ...
   standup starts from it, and check nothing off for it. Degrade as standup
   does. `gh` failing means `trackers: NOT READ → <reason>` in the seal, never a
...
3. **Seals the session's daylog.** Appends the close: what moved, what carries
   ...
   **Why an intent check and not a state check.** 2026-08-10 was sealed at
   ...
   then ran seven more hours. That is a third failure shape, distinct from
   cold-sealing and from never-closing: *a day that closed early*. The obvious
   fix, refusing to seal while the session is open, **cannot be built.** This is
   ...
   **A scoped run does not seal.** *Moved* and *carries to tomorrow* are claims
   ...
   **Mark unwitnessed work.** A block covering work the human did not watch, say
   an agent running while they slept, carries an explicit `unwitnessed` marker,
   or three weeks later it reads as their own reasoning. Conversely,
   **first-hand testimony is evidence**: record what the human says happened,
   attribute it as testimony, and let it outrank silence in the git record.
   `git` proves what landed and when, never who was at the desk.
...
In either case, append **three lines to the open daylog**: what I'm mid-way
through · what I just decided and why · what I'd start with next. The full seal
waits for the actual stop.
