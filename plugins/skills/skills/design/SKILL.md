...
**Interface**: everything a caller must know to use the module. That means
entry points and types, plus invariants, ordering, and error modes. The
...
production, an in-memory adapter in tests). Counting adapters prices a seam:
one is hypothetical, two make it real.
...
- [deepening.md](deepening.md): how to deepen a shallow cluster safely,
  covering the dependency taxonomy (which category ⇒ which test strategy),
  seam discipline, and replace-don't-layer testing. For `refactor` and `audit`.
