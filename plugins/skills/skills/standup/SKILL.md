...
name: standup
description: >-
  Log-in briefing. Bare, it spans every project: where each stands and where the
  next move has most leverage. It refreshes the rolling hq/SITREP.md and opens
  today's daylog. Given a project or target, it briefs only that.
---
...
2. **Reads the trackers: one call, never a repo list.**
   ...
   **Never enumerate repos.** A hand-built list is a document, and documents go
   ...
   still missed `super-solarized#1`, in a repo that `devex/HEAD.md` had been
   listing since that morning. `@me` has no list to keep true and costs about a
   second.
...
4. **Opens today's daylog**: `hq/<YYYY-MM-DD>-daylog.md` (`artifact: daylog`),
   seeded with the day's plan drawn from the sitrep. A scoped run after the
   morning's bare one finds today's daylog already open. Append the new briefing
