...
Claude frontmatter remains canonical Claude metadata. For every skill with
`disable-model-invocation: true`, also add `agents/openai.yaml` with
`policy.allow_implicit_invocation: false`; this is Codex's native projection of
the same human-only boundary. Codex has no direct equivalent for hiding an
explicit skill via `user-invocable: false`, nor for skill-level `model` and
`effort` pins. Keep those Claude fields intact and record Codex tiers in
`harness/models.json`; do not rewrite the skill's behavior around the gap.
